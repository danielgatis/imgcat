...
func disableEcho() *unix.Termios {
	termios, err := unix.IoctlGetTermios(int(os.Stdout.Fd()), ioctlReadTermios)
	if err != nil {
	...
	if err := unix.IoctlSetTermios(int(os.Stdout.Fd()), ioctlWriteTermios, &newState); err != nil {
...
func enableEcho(termios *unix.Termios) {
	if err := unix.IoctlSetTermios(int(os.Stdout.Fd()), ioctlWriteTermios, termios); err != nil {
		log.Fatalf("failed to set the termios: %v", err)
	}
}
...
func main() {
	...
	flaggy.DefaultParser.Version = "1.0.0"
